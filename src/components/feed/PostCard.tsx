      ...
      <div className="bento-card gradient-border">
        <div className="flex items-center gap-3 p-5 pb-3">
          ...
          <div className="flex items-center gap-1">
            {canEdit && onEdit && (
              <button onClick={() => setEditing(!editing)} className="p-2.5 sm:p-2 rounded-full text-muted-foreground hover:text-foreground hover:bg-white/40 dark:hover:bg-white/5 transition-colors active:scale-95" aria-label="Редактировать">
            ...
            {canDelete && onDelete && (
              <button onClick={() => onDelete(post.id)} disabled={isDeleting} className="p-2.5 sm:p-2 rounded-full text-muted-foreground hover:text-red-500 hover:bg-red-500/10 transition-colors disabled:opacity-30 disabled:cursor-not-allowed active:scale-95" aria-label="Удалить">
                {isDeleting ? (
        ...
        <div className="flex items-center gap-1 px-5 py-3 border-t border-border/40">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 sm:px-3 sm:py-1.5 rounded-full text-sm transition-all duration-200 disabled:opacity-50 active:scale-95 ${
              post.isLiked
          ...
          <button onClick={() => setShowComments(!showComments)} className={`flex items-center gap-1.5 px-4 py-2 sm:px-3 sm:py-1.5 rounded-full text-sm transition-all duration-200 active:scale-95 ${
            showComments
          ...
          <div className="px-5 pb-5 pt-1 space-y-3 bg-gradient-to-b from-transparent via-amber-50/30 to-amber-50/50 dark:via-amber-500/3 dark:to-amber-500/5 rounded-b-3xl">
            ...
            <div className="flex gap-2 items-center pt-1">
              <input
                ...
                className="flex-1 px-5 py-3 sm:px-4 sm:py-2.5 rounded-full bg-white/70 dark:bg-white/5 outline-none text-sm border border-border/30 focus:border-amber-300/50 transition-all"
              />
              <button
                onClick={handleComment}
                disabled={!commentText.trim() || sending}
                className="p-3 sm:p-2.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-white disabled:opacity-50 transition-all shadow-sm shadow-amber-200/30 hover:shadow-amber-200/50 shrink-0 active:scale-95"
              >
