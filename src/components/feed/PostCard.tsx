...
export function PostCard({ post, currentUserId, onToggleLike, onComment, onDelete }: PostCardProps) {
    ...
    <div className="glass-card overflow-hidden relative group">
      {canDelete && onDelete && (
        <button
          ...
          className="absolute top-3 right-3 p-1.5 rounded-lg bg-red-500/10 text-red-500 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500/20 z-10"
        >
      ...
      {showComments && (
        <div className="p-4 pt-3 space-y-3">
          <div className="flex gap-3">
            ...
            />
            <button onClick={handleComment} disabled={!commentText.trim() || sending} className="px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-semibold disabled:opacity-50">
              Отправить
            </button>
          </div>
        </div>
