    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/80 backdrop-blur-xl">
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-56 glass-card p-2 z-50">
                  ...
                  <Link href="/admin" className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm hover:bg-accent transition-colors" onClick={() => setMenuOpen(false)}>Админ-панель</Link>
      ...
      {searchOpen && (
        <div className="lg:hidden px-4 pb-3">
          <input type="search" placeholder="Поиск..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-accent outline-none text-sm" autoFocus />
        </div>
      )}
    </header>
  );
