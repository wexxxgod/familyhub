    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/80 backdrop-blur-xl">
      <div className="flex items-center justify-between h-16 px-4 lg:px-6">
        ...
        <div className="hidden sm:block relative max-w-md w-full mx-4">
          ...
          <input
            type="search"
            placeholder="РџРѕРёСЃРє РїРѕ СЃР°Р№С‚Сѓ..."
            value={searchQuery}
          ...
          {searchResults && searchQuery.trim() && (
            <div className="absolute top-full mt-2 left-0 right-0 glass-card p-2 z-50 max-h-80 overflow-y-auto">
              {searchResults.posts?.length > 0 && <p className="text-xs text-muted-foreground px-3 pt-2 pb-1">РџРѕСЃС‚С‹</p>}
              ...
              {searchResults.users?.length > 0 && <p className="text-xs text-muted-foreground px-3 pt-2 pb-1">РџРѕР»СЊР·РѕРІР°С‚РµР»Рё</p>}
              {searchResults.users?.slice(0, 3).map((u: any) => (
                <div key={u.id} className="px-3 py-2 text-sm">{u.name}</div>
              ))}
              {searchResults.archive?.length > 0 && <p className="text-xs text-muted-foreground px-3 pt-2 pb-1">РђСЂС…РёРІ</p>}
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            <button onClick={() => setNotifOpen(!notifOpen)} className="relative p-2 rounded-xl hover:bg-accent transition-colors">
              ...
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-80 glass-card p-2 z-50 max-h-96 overflow-y-auto">
                  <div className="flex items-center justify-between px-3 py-2 border-b border-border mb-2">
                    <p className="font-semibold text-sm">РЈРІРµРґРѕРјР»РµРЅРёСЏ</p>
                    {unreadCount > 0 && (
                      <button onClick={() => { markAllRead(); }} className="text-xs text-primary hover:underline">РџСЂРѕС‡РёС‚Р°С‚СЊ РІСЃРµ</button>
                    )}
                  </div>
                  {notifications.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-6">РќРµС‚ СѓРІРµРґРѕРјР»РµРЅРёР№</p>
                  ) : (
                    notifications.map((n) => (
                      <button
                        key={n.id}
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-56 glass-card p-2 z-50">
                  ...
                  <Link href="/member" className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm hover:bg-accent transition-colors" onClick={() => setMenuOpen(false)}>РњРѕР№ РїСЂРѕС„РёР»СЊ</Link>
                  <button onClick={() => handleLogout} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-red-500 hover:bg-red-500/10 transition-colors w-full">Р’С‹Р№С‚Рё</button>
                </div>
              </>
            )}
          </div>
      ...
      {searchOpen && (
        <div className="lg:hidden px-4 pb-3">
          <input type="search" placeholder="РџРѕРёСЃРє..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-accent outline-none text-sm" autoFocus />
        </div>
      )}
    </header>
  );
